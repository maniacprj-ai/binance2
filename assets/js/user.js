...
function edit(key) {
 ...
 // var withdraw = firebase.database().ref().child("widthdraw_code").child(key);
  ...
  var electric = firebase.database().ref().child("electric_bill").child(key);
  ...
  signature.on("value", function (snapshot4) {
    ...
    electric.on("value", function (snapshot4) {
    if (snapshot4.exists()) {


      stimage7 = snapshot4.val().image;

    }

    else {
      stimage7 = 'assets/img/no-image-available.jpg';

    }


    document.getElementById("fron51").src = stimage7;
